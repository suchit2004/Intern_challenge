...
## 🎥 5-Minute Presentation Video Script Outline

Use this outline to record a professional presentation of your challenge submission:

### ⏱️ Minute 0:00 - 0:45 | Objective
* **Screen**: Browser displaying `http://localhost:3000` with the compiler dashboard.
* **Key Points**: Explain the goal (translating natural language into executable schemas), and state that it is designed as a system reliability problem rather than a simple prompt-engineering wrapper.

### ⏱️ Minute 0:45 - 2:00 | The Multi-Stage Pipeline
* **Screen**: Input prompt: *"Build a CRM with contact registry and payments gating"*, click **Compile App**, and highlight the progress log feed.
* **Key Points**: Describe how dividing compilation into separate stages (Intent Extractor, System Design, Schema Generator) keeps contexts small, resulting in much cleaner, less hallucinated schemas.

### ⏱️ Minute 2:00 - 3:15 | Semantic Linking & Auto-Repair
* **Screen**: Scroll through the tabs (DB, API, UI, Auth). Click "View Payload Details" in the logs.
* **Key Points**: Introduce the programmatic validator. Explain that if an inconsistency (like a form widget submitting to an undefined route) is caught, the Auto-Repair engine feeds the specific error back to the LLM to get a precise, validated patch.

### ⏱️ Minute 3:15 - 4:15 | Sandbox Interpreter Preview
* **Screen**: Interact with the preview. 
  1. Open the "Contacts" page, type details, and submit. Show the new row populating both the UI table and the **Local Database Monitor** drawer.
  2. Switch role in the dropdown to "Guest" -> watch restricted pages block access.
  3. Switch to "Member", click "Upgrade to Premium", confirm mock checkout, and show gated pages immediately unlocking.
* **Key Points**: Demonstrate that the generated JSON config directly instructs the interpreter on how to build widgets, handle CRUD operations, and enforce RBAC and payments.

### ⏱️ Minute 4:15 - 5:00 | Benchmarks & Conclusion
* **Screen**: Click **Run Eval Suite** or show the completed evaluation table report.
* **Key Points**: Highlight testing across 20 prompts (10 products + 10 edge cases). Explain cost-control strategies (using Llama-3.3 on Groq to get high quality at a fraction of GPT-4o's operating cost).
