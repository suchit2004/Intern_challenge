...
function validateSchema(schema) {
  ...
  pages.forEach(page => {
    ...
    widgets.forEach(widget => {
      ...
      // Check bound API endpoints
      if (widget.dataSourceApi) {
        if (!apiOnlyPaths.has(widget.dataSourceApi)) {
          errors.push(`Widget '${widget.title}' on page '${page.name}' binds to dataSourceApi '${widget.dataSourceApi}' which does not exist in the API schema.`);
        }
      }
