...
function renderFormWidget(widget, container) {
  ...
  form.onsubmit = (e) => {
    ...
    // Find endpoint definition to match method (usually POST)
    ...
    if (res.success) {
      form.reset();
      renderPageContent(activePage); // Refresh UI (will update metric cards and tables)
    } else {
...
document.getElementById('pay-confirm-btn').onclick = () => {
  ...
  if (pg && pg.premiumRole) {
    ...
    // Check if role is in dropdown, if not add it
    let exists = false;
...
document.getElementById('compile-btn').onclick = async () => {
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      // Update Latencies & Repair counts stats
      ...
      // Populate Role Selector Dropdown in header
      const roleSelect = document.getElementById('sim-role-select');
      ...
      // Clear local storage and initialize database structure
      localStorage.clear();
      initLocalDb(compiledSchema.db_schema);

      // Execute & Draw App Inside Simulator
